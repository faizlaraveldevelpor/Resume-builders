...
import {SortableBox2} from './Dragable2';
...
function Template2({
  ...
  useEffect(()=>{
    if (SummaryDelete==true) {
setDrag((perv)=>perv.filter((data)=>data?.id!=="persnalInformation"))
      
    }
    if (SkillsDelete==true) {
setDrag((perv)=>perv.filter((data)=>data?.id!=="skills"))
      
    }
    if (ExperienceDelete==true) {
setDrag((perv)=>perv.filter((data)=>data?.id!=="experience"))
      
    }
    if (EducationDelete==true) {
setDrag((perv)=>perv.filter((data)=>data?.id!=="education"))
      
    }
  },[SummaryDelete,SkillsDelete,ExperienceDelete,EducationDelete])
          ...
          <SortableContext items={Drag.map(b => b.id)} strategy={verticalListSortingStrategy}>
            <div className="flex flex-col gap-4 h-fit">
              {Drag.map((d) => (
                ...
                setSummaryDelete={setSummaryDelete}
                SkillsDelete={SkillsDelete} 
                setSkillsDelete={setSkillsDelete}
                ExperienceDelete={ExperienceDelete} 
                setExperienceDelete={setExperienceDelete}
                EducationDelete={EducationDelete}
                 setEducationDelete={setEducationDelete}
